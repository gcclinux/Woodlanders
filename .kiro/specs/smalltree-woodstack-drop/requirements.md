...
This feature implements a dual-item random drop system for SmallTree destruction. When a SmallTree is destroyed, it will drop two items randomly selected from three possible combinations: 2x TreeSapling, 2x WoodStack, or 1x TreeSapling + 1x WoodStack. Each combination has an equal 33% probability. This follows the same random drop pattern used by BambooTree (which drops BambooStack and BambooSapling).
...
- **TreeSapling**: An item entity representing a young tree sapling that can be planted, extracted from the sprite sheet with 64x64 dimensions, rendered at 32x32 pixels
...
2. THE Game System SHALL spawn two TreeSapling items at the tree's base position when the first drop combination is selected
...
4. THE Game System SHALL spawn one TreeSapling item and one WoodStack item at the tree's base position when the third drop combination is selected
...
**User Story:** As a player, I want to see a new TreeSapling item that can be collected and used for planting, so that I can grow new trees

#### Acceptance Criteria

1. THE TreeSapling Class SHALL extract its texture from the sprite sheet with source dimensions 64x64
2. THE TreeSapling Class SHALL provide getters for texture and position coordinates
3. THE TreeSapling Class SHALL implement a dispose method for texture cleanup
4. THE MyGdxGame Class SHALL render TreeSapling items at 32x32 pixels on screen
5. THE MyGdxGame Class SHALL maintain a collection for TreeSapling items to enable rendering and collision detection
6. THE MyGdxGame Class SHALL render TreeSapling items within the camera viewport using the same culling optimization as other items
...
**User Story:** As a player, I want to be able to pick up both TreeSapling and WoodStack items by walking over them, so that I can collect the resources

#### Acceptance Criteria

1. WHEN the player's collision box overlaps with a TreeSapling item, THE Player Class SHALL remove the TreeSapling from the game world
...
3. THE Player Class SHALL check for TreeSapling and WoodStack pickups during each update cycle using the same collision detection pattern as other items
4. WHEN a TreeSapling or WoodStack is picked up in multiplayer mode, THE Game System SHALL send an item pickup message to the server
...
**User Story:** As a player, I want TreeSapling items to be added to my inventory when picked up, so that I can use them for planting later

#### Acceptance Criteria

1. WHEN a TreeSapling item is picked up, THE Inventory System SHALL add one TreeSapling to the player's inventory
2. THE Inventory System SHALL support the TreeSapling item type in the ItemType enumeration
3. THE Inventory System SHALL display the TreeSapling count in the inventory UI
4. THE Inventory System SHALL allow the player to select TreeSapling from the inventory for planting
5. WHEN the player saves the game, THE World Save System SHALL persist the TreeSapling inventory count
6. WHEN the player loads a saved game, THE World Save System SHALL restore the TreeSapling inventory count

### Requirement 6

**User Story:** As a multiplayer client, I want TreeSapling items to synchronize correctly across all players, so that everyone sees the same game state

#### Acceptance Criteria

1. THE ItemType Network Enum SHALL include a BABY_TREE entry for network synchronization
2. WHEN a TreeSapling is spawned in multiplayer mode, THE Game Server SHALL broadcast an ItemSpawnMessage with type BABY_TREE
3. WHEN a TreeSapling is picked up in multiplayer mode, THE Game Server SHALL broadcast an ItemPickupMessage for the TreeSapling
4. WHEN a client receives a TreeSapling spawn message, THE Game Client SHALL create a TreeSapling instance at the specified position
5. WHEN a client receives a TreeSapling pickup message, THE Game Client SHALL remove the TreeSapling from the local collection
