...
- [x] 1. Verify and update TreeSapling item class

  - Verify TreeSapling.java exists in src/main/java/wagemaker/uk/items/
...
- [x] 2. Add TreeSapling and WoodStack collections to MyGdxGame

  - Add TreeSapling and WoodStack import statements at top of MyGdxGame.java
  - Add `Map<String, TreeSapling> treeSaplings` field declaration with other item collections
  - Add `Map<String, WoodStack> woodStacks` field declaration with other item collections
  - Initialize `treeSaplings = new HashMap<>()` in create() method
  - Initialize `woodStacks = new HashMap<>()` in create() method
  - Wire treeSaplings collection to player with `player.setTreeSaplings(treeSaplings)` in create()
  - Wire woodStacks collection to player with `player.setWoodStacks(woodStacks)` in create()
  - _Requirements: 2.4, 2.5, 3.3_

- [x] 3. Implement TreeSapling and WoodStack rendering in MyGdxGame

  - Create `drawTreeSaplings()` method with viewport culling logic
  - Render TreeSapling items at 32x32 pixels using batch.draw()
  - Create `drawWoodStacks()` method with viewport culling logic
  - Render WoodStack items at 32x32 pixels using batch.draw()
  - Call `drawTreeSaplings()` in render() method after other item rendering
  - Call `drawWoodStacks()` in render() method after drawTreeSaplings()
  - _Requirements: 2.4, 3.2, 3.4_

- [x] 4. Add TreeSapling and WoodStack support to Player class

  - Add TreeSapling and WoodStack import statements at top of Player.java
  - Add Random import statement for random drop selection
  - Add `Map<String, TreeSapling> treeSaplings` field declaration
  - Add `Map<String, WoodStack> woodStacks` field declaration
  - Add `Random random = new Random()` field declaration
  - Implement `setTreeSaplings()` setter method
...
- [x] 5. Implement SmallTree random drop logic
  ...
  - Implement case 0: spawn 2x TreeSapling at tree position and position + 8px
  - Implement case 1: spawn 2x WoodStack at tree position and position + 8px
  - Implement case 2: spawn 1x TreeSapling and 1x WoodStack at tree position and position + 8px
...
- [x] 6. Implement TreeSapling pickup mechanics

  - Create `checkTreeSaplingPickups()` method in Player class
  - Implement collision detection with 32-pixel pickup range
  - Create `pickupTreeSapling()` method to handle pickup logic
  ...
  - Call `checkTreeSaplingPickups()` in Player.update() method
...
- [x] 10. Update multiplayer item spawn handling for TreeSapling
  ...
  - Add BABY_TREE case to create TreeSapling instances from spawn messages
  - Add items to appropriate collections (treeSaplings or woodStacks)
  - _Requirements: 6.2, 6.3, 6.4_

- [x] 11. Update multiplayer item pickup handling for TreeSapling
  ...
  - Add BABY_TREE case to remove TreeSapling from collection and add to inventory
  - Verify inventory synchronization for BABY_TREE items
  - _Requirements: 6.5, 5.1_

- [x] 12. Verify world save/load support for TreeSapling inventory
  ...
  - Test save game with TreeSapling items in inventory
  - Test load game and verify TreeSapling count is restored
...
- [x] 13. Test single-player random drop functionality
  ...
  - Verify all three drop combinations appear (2x TreeSapling, 2x WoodStack, 1x each)
  - Verify items render at 32x32 pixels
  - Verify items are positioned 8 pixels apart
  - Verify console logs show correct drop types
  - Walk over TreeSapling items and verify pickup + inventory increase
  - Walk over WoodStack items and verify pickup
  - Save game and verify TreeSapling count persists
  - Load game and verify TreeSapling count restored
